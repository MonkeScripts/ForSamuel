...
class NFromBlackboard(auv_goto.NFromBlackboard):
    """
    Sequential N-pose Locomotion action client for BlueROV2, reading list[PoseStamped] from blackboard.

    Identical to auv.goto.NFromBlackboard except:
    - Connects to /bluerov/controls (BlueROVSharedAction.LOCOMOTION)
    - Uses /bluerov/convert_to_controls_pose for frame conversion
    - anchor_frame_name defaults to 'base_link'
    - Relaxed tolerances for MAVROS-based control

    Inherits _start_goal/initialise/update from auv_goto.NFromBlackboard so the
    wait-between-moves sequencing behaviour is unchanged.
    """
    ...
    def __init__(
        ...
        # Bypass auv_goto.NFromBlackboard.__init__ (which would chain through
        # auv_goto.FromBlackboard.__init__ and bind AUVSharedAction.LOCOMOTION).
        # Call goto_base.FromBlackboard.__init__ directly with BlueROV registries.
        goto_base.FromBlackboard.__init__(
            self,
            name=name,
            pose_key=pose_key,
        ...
        # Attributes auv_goto.FromBlackboard normally sets — required by
        # inherited _gen_goal/_initialise_relative/_initialise_absolute.
        self.specified_heading = specified_heading
        self.ignore_depth = ignore_depth
        self.x_threshold = x_threshold
        self.y_threshold = y_threshold
        ...
        # Attributes auv_goto.NFromBlackboard adds for multi-pose sequencing.
